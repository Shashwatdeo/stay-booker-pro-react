...
const HotelsSearch = () => {
  ...
  const fetchHotels = useCallback(async (filters) => {
    ...
    if (hotelsResultsResponse) {
      const hotelsData =
        ...
        [];
      const metadata = hotelsResultsResponse.data?.metadata || hotelsResultsResponse.metadata || {};
      const pagination = hotelsResultsResponse.data?.paging || hotelsResultsResponse.paging || hotelsResultsResponse.paging || {};
      setHotelsResults({
        isLoading: false,
        data: hotelsData,
  ...
  const fetchAvailableCities = async () => {
    ...
    } else if (Array.isArray(availableCitiesResponse?.elements)) {
      setAvailableCities(availableCitiesResponse.elements);
    } else if (Array.isArray(availableCitiesResponse?.data?.elements)) {
  ...
  const getVerticalFiltersData = async () => {
    ...
    const rawElements = Array.isArray(filtersDataResponse?.elements)
      ? filtersDataResponse.elements
      : Array.isArray(filtersDataResponse?.data?.elements)
        ? filtersDataResponse.data.elements
        : Array.isArray(filtersDataResponse)
          ? filtersDataResponse
          : [];

    // Transform backend response to match frontend expectations
    const elements = rawElements.map(filterGroup => ({
  ...
  useEffect(() => {
    ...
    fetchHotels({ city: locationInputValue });
  }, []);
