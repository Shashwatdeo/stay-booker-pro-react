...
const Checkout = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Debug log the raw response so we can see reasons for failure in dev tools
    // eslint-disable-next-line no-console
    console.debug('Payment confirmation response:', response);

    const succeeded = !!(
      response && (response.success === true || (response.data && (!response.errors || response.errors.length === 0)))
    );
    ...
    if (succeeded) {
      ...
      const bookingPayload = {
        userId: userDetails?.id || 1, // fallback to 1 if not logged in
      ...
      await networkAdapter.post('/bookings', bookingPayload);
      ...
      const serverError = (response && response.errors && response.errors[0]) || response.message || (response && response.data && response.data.message);
      setToastMessage(serverError || 'Payment failed. Please try again.');
...
const validationSchema = {
  ...
  cardNumber: (value) => {
    // Remove spaces and check if it has exactly 16 digits
    const digitsOnly = value.replace(/\s/g, '');
    return /^\d{16}$/.test(digitsOnly);
  }, // Check if it has 16 digits after removing spaces
  ...
  postalCode: (value) => /^\d{5}(-\d{4})?$/.test((value || '').trim()), // US postal code format: 12345 or 12345-6789
};

export default Checkout;

export { InputField, validationSchema };
