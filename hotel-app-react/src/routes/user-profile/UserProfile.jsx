  ...
  useEffect(() => {
    const getInitialData = async () => {
      try {
        ...
        // Normalize bookings response from different backends/mocks
        let bookings = [];
        let bookingsErrors = [];
        if (userBookingsDataResponse) {
          // Mirage shape: { errors: [], data: { elements: [...] } }
          if (userBookingsDataResponse.data && Array.isArray(userBookingsDataResponse.data.elements)) {
            bookings = userBookingsDataResponse.data.elements;
          } else if (Array.isArray(userBookingsDataResponse.bookings)) {
            // legacy/backend shape: { bookings: [...] }
            bookings = userBookingsDataResponse.bookings;
          } else if (Array.isArray(userBookingsDataResponse.data)) {
            bookings = userBookingsDataResponse.data;
          }
          bookingsErrors = userBookingsDataResponse.errors || [];
        }
        ...
        let payments = [];
        let paymentsErrors = [];
        if (userPaymentMethodsResponse) {
          if (userPaymentMethodsResponse.data && Array.isArray(userPaymentMethodsResponse.data.elements)) {
            payments = userPaymentMethodsResponse.data.elements;
          } else if (Array.isArray(userPaymentMethodsResponse.data)) {
            payments = userPaymentMethodsResponse.data;
          }
          paymentsErrors = userPaymentMethodsResponse.errors || [];
        }
        ...
        setUserBookingsData({ isLoading: false, data: [], errors: [err.message] });
        setUserPaymentMethodsData({ isLoading: false, data: [], errors: [err.message] });
      }
    };
    getInitialData();
  }, [userDetails]);
