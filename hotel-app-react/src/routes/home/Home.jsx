...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const getInitialData = async () => {
      ...
      // TODO: Implement or remove popularDestinations if backend supports it

      const hotelsResultsResponse = await networkAdapter.get('/hotels');

      // const availableCitiesResponse = await networkAdapter.get('/hotels/cities');
      // if (Array.isArray(availableCitiesResponse)) {
      //   setAvailableCities(availableCitiesResponse);
      // } else if (availableCitiesResponse && availableCitiesResponse.data && Array.isArray(availableCitiesResponse.data.elements)) {
      ...
      if (hotelsResultsResponse) {
        ...
        const hotelsData =
          (hotelsResultsResponse.data && Array.isArray(hotelsResultsResponse.data.elements) && hotelsResultsResponse.data.elements) ||
          hotelsResultsResponse.hotels ||
          [];
        setHotelsResults({
          isLoading: false,
          data: hotelsData,
        ...
        // debug: log parsed hotels data
        // eslint-disable-next-line no-console
        console.info('Parsed hotels data for state:', hotelsData.length, hotelsData[0] || null);
        // Populate popular destinations with static CDN images (reliable placeholders)
        setPopularDestinationsData({
          isLoading: false,
          data: [
            { code: 1, name: 'Bangkok', imageUrl: '/images/cities/bangkok.jpg' },
            { code: 2, name: 'Dubai', imageUrl: '/images/cities/dubai.jpg' },
            { code: 3, name: 'London', imageUrl: '/images/cities/london.jpg' },
            { code: 4, name: 'Mumbai', imageUrl: '/images/cities/mumbai.jpg' },
            { code: 5, name: 'Oslo', imageUrl: '/images/cities/oslo.jpg' },
          ],
          errors: [],
        });
      }
