...
const HotelBookingDetailsCard = ({ hotelCode }) => {
  ...
  const guestOptions = Array.from(
    { length: bookingDetails.maxGuestsAllowed || 2 },
    (_, i) => ({ value: i + 1, label: `${i + 1} guest` })
  );
  const roomNumberOptions = Array.from(
    { length: bookingDetails.maxRoomsAllowedPerGuest || 1 },
        ...
        <div className="p-6 text-sm md:text-base">
          <div className="text-center py-8">
            <div className="mb-4">
              ...
              <p className="text-gray-600 mb-4">Please login to book this hotel</p>
            </div>
            <button
              onClick={() => navigate('/login')}
              className="w-full bg-brand text-white py-2 px-4 rounded hover:bg-blue-700 transition duration-300"
            >
              Login to Book
            </button>
          </div>
